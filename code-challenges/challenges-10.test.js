...
const createServer = () => {
  ...
  app.get('/hello', (request, response) => {
    response.status(200).send('Hail and good day human');
  });
  app.get('/aboutme', (request, response) => {
    response.status(200).send(`I'm a computer communicating through a terminal`);
  });
  app.get('/favoritefoods', (request, response) => {
    response.status(200).send(['microchips', 'small bytes']);
  });
